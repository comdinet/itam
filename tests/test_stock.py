...
mice = stock.create("Logitech M185 mouse", "Peripheral", 2500, 10, vendor="Logitech")
item = stock.get(mice)
s = stock.summary(item)
...
check("handing out one succeeds", stock.assign(mice, "ada@x.com", 1), None)
check("handing out three more to someone else", stock.assign(mice, "grace@x.com", 3), None)
s = stock.summary(stock.get(mice))
...
check("second unit to Ada", stock.assign(mice, "ada@x.com", 1), None)
check("still two holders", len(stock.summary(stock.get(mice))["holders"]), 2)
check("Ada now holds 2",
      db.q1("SELECT quantity FROM stock_allocations WHERE item_id=? AND upn='ada@x.com'",
            (mice,))["quantity"], 2)

print("\n--- cost follows the units ---")
ada = [k for k in stock.for_user("ada@x.com")][0]
check("Ada carries 2 x 25.00", ada["cost"], 5000)
grace = [k for k in stock.for_user("grace@x.com")][0]
check("Grace carries 3 x 25.00", grace["cost"], 7500)

print("\n--- you cannot hand out what you do not have ---")
problem = stock.assign(mice, "hedy@x.com", 99)
check("refused", problem is not None, True)
check("says how many are available", "5 unit(s) available" in problem, True)
check("nothing changed", stock.summary(stock.get(mice))["allocated"], 5)

print("\n--- taking back ---")
check("take one back from Grace", stock.take_back(mice, "grace@x.com", 1), None)
check("Grace holds 2",
      db.q1("SELECT quantity FROM stock_allocations WHERE item_id=? AND upn='grace@x.com'",
            (mice,))["quantity"], 2)
check("take back all of Grace's", stock.take_back(mice, "grace@x.com"), None)
check("Grace holds none",
      db.q1("SELECT COUNT(*) c FROM stock_allocations WHERE item_id=? AND upn='grace@x.com'",
            (mice,))["c"], 0)
check("taking back from someone with none is refused",
      stock.take_back(mice, "grace@x.com") is not None, True)

print("\n--- quantity owned cannot drop below what is handed out ---")
s = stock.summary(stock.get(mice))
problem = stock.update(mice, "Logitech M185 mouse", "Peripheral", 2500, 1, None, None)
check("refused", problem is not None, True)
check("mentions how many are out", "2 unit(s) are already handed out" in problem, True)
check("quantity unchanged", stock.get(mice)["quantity"], 10)
check("raising it is fine",
      stock.update(mice, "Logitech M185 mouse", "Peripheral", 2500, 20, None, None), None)
check("now 20 owned", stock.get(mice)["quantity"], 20)

print("\n--- the JetBrains case: licences bought once, handed to several people ---")
jb = stock.create("JetBrains All Products Pack", "Software", 77900, 4, vendor="JetBrains")
for upn in ("ada@x.com", "grace@x.com", "hedy@x.com"):
    check(f"seat to {upn}", stock.assign(jb, upn, 1), None)
sj = stock.summary(stock.get(jb))
...
check("a fourth is fine", stock.assign(jb, "ada@x.com", 1), None)
check("a fifth is refused", stock.assign(jb, "grace@x.com", 1) is not None, True)
...
ada_total = sum(k["cost"] for k in stock.for_user("ada@x.com"))
...
check("allocations gone",
      db.q1("SELECT COUNT(*) c FROM stock_allocations WHERE item_id=?", (jb,))["c"], 0)
check("the other item is untouched", stock.get(mice)["quantity"], 20)
...
check("their allocations are removed",
      db.q1("SELECT COUNT(*) c FROM stock_allocations WHERE upn='ada@x.com'")["c"], 0)
check("units return to stock", stock.summary(stock.get(mice))["available"], 20)
